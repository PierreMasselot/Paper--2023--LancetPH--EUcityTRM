...
# Define reference percentiles for heat and cold RRs
refper <- c(25, 75)
...
erf <- function(blup, cen = NULL){
  firstpred <- ov_basis %*% blup$blup
  if (is.null(cen)){
    cen <- predper[inrange][which.min(firstpred[inrange,])]
  }
  crosspred(basis = ov_basis, coef = blup$blup, vcov = blup$vcov, 
    cen = cen, at = predper, model.link = "log")
}

#----- Extract ERFs 

# Extract continental level ERF: cold
continentERF_cold <- lapply(stage2res, lapply, lapply, function(x) {
  blext <- blup(x, level = 0, vcov = T)[[1]]
  erf(blext, cen = 25)
})

# Extract continental level ERF: heat
continentERF_heat <- lapply(stage2res, lapply, lapply, function(x) {
  blext <- blup(x, level = 0, vcov = T)[[1]]
  erf(blext, cen = 75)
})

# Extract continental level ERF: MMP
continentERF_mmp <- lapply(stage2res, lapply, lapply, function(x) {
  blext <- blup(x, level = 0, vcov = T)[[1]]
  erf(blext)
})
